...
def set_system_volume(percent: int) -> Dict[str, Any]:
    if sys.platform != "linux":
        return {"status": "error", "message": "Volume control is currently only supported on Linux/Ubuntu."}
    try:
        ...
        return {"status": "error", "message": f"Failed to set volume: {e.stderr.decode()}"}
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
def set_system_brightness(percent: int) -> Dict[str, Any]:
    if sys.platform != "linux":
        return {"status": "error", "message": "Brightness control is currently only supported on Linux/Ubuntu."}
    try:
        ...
        subprocess.run([
            ...
            "org.gnome.SettingsDaemon.Power.Screen", "Brightness", f"<int32 {percent}>"
        ], check=True, capture_output=True)
        return {"status": "success", "message": f"Brightness set to {percent}%"}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Failed to set brightness via dbus: {e.stderr.decode()}"}
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
def set_system_theme(theme: str) -> Dict[str, Any]:
    if sys.platform != "linux":
        return {"status": "error", "message": "Theme control is currently only supported on Linux/Ubuntu."}
    try:
        ...
        return {"status": "error", "message": f"Failed to set theme: {e.stderr.decode()}"}
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
def manage_wifi(action: str) -> Dict[str, Any]:
    if sys.platform != "linux":
        return {"status": "error", "message": "Wi-Fi control is currently only supported on Linux/Ubuntu."}
    try:
        if action not in ["on", "off"]:
            return {"status": "error", "message": "Action must be 'on' or 'off'."}
        subprocess.run(["nmcli", "radio", "wifi", action], check=True, capture_output=True)
        return {"status": "success", "message": f"Wi-Fi turned {action}."}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Failed to modify Wi-Fi state: {e.stderr.decode()}"}
...
def manage_bluetooth(action: str) -> Dict[str, Any]:
    if sys.platform != "linux":
        return {"status": "error", "message": "Bluetooth control is currently only supported on Linux/Ubuntu."}
    try:
        if action not in ["on", "off"]:
            return {"status": "error", "message": "Action must be 'on' or 'off'."}
        rfkill_action = "block" if action == "off" else "unblock"
        subprocess.run(["rfkill", rfkill_action, "bluetooth"], check=True, capture_output=True)
        return {"status": "success", "message": f"Bluetooth turned {action}."}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Failed to modify Bluetooth state: {e.stderr.decode()}"}
    except FileNotFoundError:
