...
class MemoryTier2:
    ...
    def record_user_response(
        ...
        event["confidence_score"] = self._calculate_confidence_score(
            event["accepted_count"], event["total_shown"]
        )

        # Update promotion status based on thresholds
        event["promotion_status"] = self._determine_promotion_status(
            event["confidence_score"], event["accepted_count"], event["rejected_count"]
        )
    ...
    def promote_candidates_to_tier3(
        ...
        results = self.collection.get(
            where={"promotion_status": {"$eq": "candidate"}}, limit=1000
        )
...
if __name__ == "__main__":
    ...
    updated = memory.record_user_response(event_id, accepted=True)
    print(
        f"After 3rd acceptance: {updated.get('confidence_score')}, Status: {updated.get('promotion_status')}"
    )
